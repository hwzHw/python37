...
X.json = function(r) {
	...
	if (type == 'alert') {
		...
		if('function' == typeof artDialog){
			artDialog.tips(data,1.5);
		}else{
			alert(data);
		}
	} else if (type == 'mix') {
...
window.x_init_hook_dealimage = function() {
	...
	//var m = teamside ? 710: 700;
	var m = 710;
	if (teamside) {
		jQuery('#team_main_side').css('width', '710px');
...
window.x_init_hook_click = function() {
	...
	jQuery('#edit-inventory').click(function() {
		X.get(WEB_ROOT + '/ajax/coupon.php?action=dialog')
	});
...
//isFindSpecialProvince 特殊地区邮费问题
function dealbuy_totalprice(isFindSpecialProvince) {
	...
	if('undefined' != typeof isFindSpecialProvince && isFindSpecialProvince){
		$('#deal-express-total').html($('.express-price:checked').val())
	}
...
window.x_init_hook_team = function() {
	$('#deal-buy-quantity-input').bind("blur",
	function() {
...
X.manage.orderremark = function() {
	...
	var oid = jQuery('#dialog-order-id').attr('oid');
	if (oid) return ! X.get(WEB_ROOT + '/ajax/manage.php?action=orderremark&id=' + oid + '&m=' + encodeURIComponent(m))
};
X.manage.orderuserremark = function() {
	var m = jQuery('#dialog_order_userremark').val();
	var oid = jQuery('#dialog-order-id').attr('oid');
	if (oid) return ! X.get(WEB_ROOT + '/ajax/manage.php?action=orderuserremark&id=' + oid + '&m=' + encodeURIComponent(m))
};
...
window.x_init_hook_editor = function() {
	...
	jQuery('textarea.editor').each(function() {/*
		KindEditor.create('#'+jQuery(this).attr('id'), {
			resizeType : 1,
			...
			items: ['source', 'textcolor', 'bgcolor', 'fontname', 'fontsize', 'bold', 'italic', 'underline', 'removeformat', '|', 'justifyleft', 'justifycenter', 'justifyright', 'insertorderedlist', 'insertunorderedlist', '|', 'image','flash', 'link', 'advtable', '|', 'fullscreen','album']
